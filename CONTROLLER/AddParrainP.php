...
require_once('../MODEL/Parrain.php');
    ...
    if(isset($_POST['btn'])){
        ...
        if($dateDC<date('Y-m-d')){
            ...
            header('Location:../VIEW/ParrainPhysique.php');
        }else if($PAR->checkMail($email)>0){
            $_SESSION['err']='Ce Mail est deja utilise';
            header('Location:../VIEW/ParrainPhysique.php');
        }else if($PAR->checkTel($telephone)>0){
            $_SESSION['err']='Ce Numero est deja utilise';
            header('Location:../VIEW/ParrainPhysique.php');
        }else{
            $_SESSION['err']="";
